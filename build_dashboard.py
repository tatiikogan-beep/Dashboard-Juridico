...
.up5{display:grid;grid-template-columns:repeat(5,minmax(110px,1fr));gap:8px;margin-bottom:16px}
...
function generate(){
  ...
  if(ST.proc && clients && !procFld) warns.push('Campo "Cliente principal" não encontrado na planilha de Processos.');
  if(ST.serv && clients && !servFld) warns.push('Campo "Cliente principal" não encontrado na planilha de Serviços.');
  ...
  if(!ST.proc){ var pb=document.getElementById('procblock'); if(pb) pb.style.display='none'; }
